<?php
    ...
    if (isset($_GET["submit"])) {
        if ( isset($_GET["user_id"]) && !empty($_GET["user_id"]) ) {
            $userId = "";
            foreach ($_GET["user_id"] as $key => $value) {
                $userId .= ",{$value}";
            }
            $userId = preg_replace("/,/", "", $userId, 1);

            $conditionGetUser .= " AND users.id IN ($userId)";
        }
        ...
        $sqlGetUser = "SELECT
                ...
                (
                    ...
                    WHERE
                        ...
                        AND users.id = donate_types.user_id
                ) AS donate_types_name
            FROM
                users
            ...
            WHERE
                ...
                .((!empty($conditionGetUser)) ? "$conditionGetUser" : "")."
                AND users.status = '1'
    ...
    // Get Users
    $sqlUsers = "SELECT * FROM users WHERE deleted_at IS NULL AND status = '1'";
    $resultUsers = mysqli_query($conn, $sqlUsers);
    if (!$resultUsers) { die("Error: " . mysqli_error($conn)); }
